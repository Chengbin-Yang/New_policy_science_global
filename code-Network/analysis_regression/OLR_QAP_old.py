...
class Config:
    ...
    FILES_X = {
        ...
        "GHG_Sim": "1-6-2-GHG_Sim_long.csv",
...
def _order_pairs(a: pd.Series, b: pd.Series) -> tuple[pd.Series, pd.Series]:
    # Try numeric ordering first (safer when ids are numeric strings like "10" vs "2")
    try:
...
def load_network_data(path: Path, value_name: str) -> pd.DataFrame:
    """
    Load edge list, canonicalize undirected dyads, build stable Key, de-duplicate by Key.
    Expected columns (case-insensitive): Source, Target, and one of [weight, value].
    """
    if not path.exists():
...
def _make_ordered_endog(y: pd.Series) -> pd.Categorical:
    """
    Ensure category order is explicit.
    If y is numeric, categories are sorted by numeric value.
    """
    y_clean = y.dropna()
...
def run_qap(df_x: pd.DataFrame, df_y: pd.DataFrame, y_col: str, x_cols: list[str]) -> pd.DataFrame:
    ...
    try:
        ...
        # Fallback using normal approximation if needed
    ...
    perm_list = Parallel(n_jobs=Config.N_JOBS, verbose=5)(
        delayed(_qap_worker)(int(s), df_x_small, df_y_small, y_col, x_cols, nodes) for s in seeds
    )
...
def plot_forest(res_df: pd.DataFrame, title: str, out_path: Path):
    ...
    # Colors reflect QAP significance
